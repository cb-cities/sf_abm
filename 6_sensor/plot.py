...
def plot_pcp(var, hour, random_seed, cov):
    ...
    parallel_coordinates(pcp_df.loc[pcp_df['highlight']=='mid', var_cols], 'highlight', color='b', alpha=0.2)
    parallel_coordinates(pcp_df.loc[pcp_df['highlight']=='top', var_cols], 'highlight', color='r', alpha=0.4)
    ...
    plt.yscale('log')
    plt.title("Change in road link usage by probe ratio, Friday {} o'clock, COV {}".format(hour, cov))
    ...
    plt.legend(
        #handles=[negHandle],
        handles=[topHandle, midHandleOne,lowHandle, negHandle], 
        title='Percentile, hourly link volume in the perfect information case',
...
if __name__ == '__main__':
    ...
    plot_pcp('true_flow', 18, 0, 0.0) ### max flow hour is 18
