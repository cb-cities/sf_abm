...
def main(year = 1):
    ...
    ### 3: SFO
    gate_id = [0, 1, 2, 3]
    gate_traffic = [55000, 125000, 100000, 70000]
    ...
    for hour in range(3, 27):
        ...
        while (len(hourly_OD_list) < hourly_demand):### While not having generated enough OD pairs
            ...
            step_demand = max(100, int(0.1*(hourly_demand-len(hourly_OD_list))))
        ...
        hourly_OD_osmid_df.to_csv(absolute_path+'/../output/{}/intercity_growth/intercity_YR{}_HR{}.csv'.format(folder, year, hour), index=False)

def check(year):

    intercity_OD_df = pd.read_csv(absolute_path+'/../output/{}/intercity_growth/intercity_YR{}_HR{}.csv'.format(folder, year, 3))
    for hour in range(4, 27):
        hourly_OD_df = pd.read_csv(absolute_path+'/../output/{}/intercity_growth/intercity_YR{}_HR{}.csv'.format(folder, year, hour))
        intercity_OD_df = pd.concat([intercity_OD_df, hourly_OD_df], ignore_index=True)

    print(intercity_OD_df.groupby(['O']).size())
    print(intercity_OD_df.groupby(['D']).size())
