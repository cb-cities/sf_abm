...
def get_grid(budget, iri_impact, case):
    all_repair_df = pd.read_csv('output_march/repair_df/repair_df_y0_c{}_b{}_i{}.csv'.format(case, budget, iri_impact))
    all_repair_df['year'] = 0

    for year in range(1, 10):
        repair_df = pd.read_csv('output_march/repair_df/repair_df_y{}_c{}_b{}_i{}.csv'.format(year, case, budget, iri_impact))
        repair_df['year'] = year
...
def grid_plot():
    ...
    a_eco = m_eco.toarray()
    a_eco_kg = np.where(a_eco>15000, 15, a_eco/1000)
    ...
    a_normal_kg = np.where(a_normal>15000, 15, a_normal/1000)

    vmin = max(a_eco_kg[:, 0:200].min(), a_normal_kg[:, 0:200].min())
    vmax = min(a_eco_kg[:, 0:200].max(), a_normal_kg[:, 0:200].max())
    ...
    plt.subplot(211)
    plt.imshow(a_normal_kg[:, 0:200], cmap=plt.cm.gnuplot_r)
    ...
    plt.subplot(212)
    plt.imshow(a_eco_kg[:, 0:200], cmap=plt.cm.gnuplot_r)
    ...
    ax = cbar.ax
    ax.text(2,0.7,'Link-level AAD-CO\u2082\n reduction (kg)', rotation=90)
    #plt.show()
